...
public class ServerBackground {
	...
	// 서버 생성 및 설정
	public void setting() {
		// 참고: 동시접속자로 map에 정보가 동기화되어 들어가도록 설정.
		Collections.synchronizedMap(mapClients);
		
		try {
			serverSocket = new ServerSocket(7777);
			// 방문자 접속을 계속 받아들임, 무한반복함, GUI 프로그램경우 창 닫힐 때까지 계속 반복됨. break 없음.
			while (true) {
				// 접속자 대기 중
	...
	class Client extends Thread {
		...
		@Override
		public void run() {
			// 클라이먼트마다 각각에서는절 전달되어오는 메시지 확인하고 화면에 출력.
			// client와 입력 통로가 끊어지지 않았다면 계쏙 반복확인함.
			// client에서 수신받은 msg
			while (br != null) {
				try {
